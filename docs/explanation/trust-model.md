...
Import and update commands plan before writing and bind the full approved plan. They copy content without running scripts or hooks. An update explicitly targets an existing skill with valid provenance, includes file removals in its review, and preserves the other skills and MCP files in the plugin. Generation prepares validated client outputs in a temporary tree before replacing the destination files. Publication records each target before moving it and retains backups until every replacement or rollback has completed. Failure tests cover errors, Ctrl-C cancellation before and after a successful move, and a second interruption during recovery. Keep any reported recovery directory until the repository has been restored; see [rollback recovery](../how-to/troubleshoot.md#marketplace-generation-reports-a-rollback-failure).

Handled cancellation during replacement attempts rollback, including restoration of the catalog. If recovery fails or is interrupted, Forge retains remaining backups and reports their location. A process kill, power loss, or cancellation after commit can still require manual inspection; these filesystem operations do not provide crash-atomic durability.
