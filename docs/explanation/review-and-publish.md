...
The saved URL is a routing preference, not approval to publish. Each import or existing-skill update produces its own review plan with the final repository URL bound into its hash. A saved preference or an old plan cannot authorize changed content or a different target. Approval to implement a local update does not by itself authorize a push, pull request, or merge.
