...
The macOS Python 3.12.13 suite passed 583 tests with three filesystem or platform-specific skips and 93.05% coverage. Ruff, formatting, ty, Forge validation, generation drift, the strict documentation build, and isolated wheel installation checks passed. Regression coverage includes MCP argument files and directories, path traversal, license evidence casing, shared provenance, rollback, YAML mappings, diagnostics outside a checkout, release source evidence, and preservation of unchanged catalog bytes. The wheel smoke imported two authored skills and updated one, ran their reviewed renderers only after package validation, and retained six verified Markdown, JSON, and SVG artifacts alongside the tested distributions. Peer review findings were corrected and verified.

With isolated local client homes, Codex CLI `0.155.1` and Copilot CLI `1.0.84-1` installed the local `1.1.0` marketplace package. Codex's cached package matched the source tree byte-for-byte; Copilot loaded the package live and listed `package-agent-skill`. These checks establish local installation and discovery. The isolated Codex home was not logged in, and Copilot reported no authentication information, so neither isolated installation performed an agent invocation. A fresh public Forge `1.1.0` marketplace installation in VS Code `1.138.0` remains pending merge.

For a separate end-to-end Codex check, Forge `1.1.0` imported the authored `release-digest` and `status-chart` fixtures into two disposable `0.1.0` plugins using separate reviewed plans. `forge generate` and `forge check` passed. Codex CLI `0.155.1` installed both from that local marketplace; both cached packages matched their portable sources byte-for-byte. Authenticated, ephemeral invocations read the installed skills and ran their reviewed renderers, producing Markdown, JSON, and SVG files that matched all four packaged expected artifacts. The JSON and SVG content was also checked semantically. Both temporary plugins and the marketplace were removed, and the client's registrations matched their pre-test snapshots. This verifies the Codex local-plugin user journey; the public `1.1.0` marketplace is still pending merge.

In a new `Forge-1.1-E2E` VS Code profile, the same two portable packages were loaded through `chat.pluginLocations`. **Chat: Configure Skills...** discovered both skills at their packaged paths. VS Code Chat then invoked each packaged renderer with its sample input; the Markdown, JSON, and SVG outputs matched all four packaged expected files by SHA-256, and the JSON and SVG content checks passed. This verifies the local-plugin skill journey in VS Code, not a fresh public marketplace installation.

The local `release-status` MCP server was discovered in VS Code `1.138.0` but did not start: VS Code passed `${PLUGIN_ROOT}/server.py` literally, so no tool call or `PLUGIN_ROOT`/`PLUGIN_DATA` environment check passed. The portable configuration is valid under Agent Plugins 1.0. This matches the open [VS Code placeholder-expansion bug](https://github.com/microsoft/vscode/issues/335006). VS Code skill invocation is qualified separately; portable MCP runtime acceptance is blocked on this client version.

The PR must pass the protected branch's required quality and Linux, macOS, and Windows jobs, plus the project's Python 3.11 release check, before merge. Release qualification still requires the remaining client acceptance and public read-back steps in the [release guide](../how-to/validate-release.md), including the blocked VS Code MCP check. These local results do not establish a fresh public marketplace installation or a Forge publication invocation from that public installation. Historical evidence below retains its original version and scope.
