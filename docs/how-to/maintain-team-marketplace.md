...
Add `--diff` to the planning command to see instruction and license text changes alongside file and executable-mode changes. It is read-only and does not change the plan hash. Review the full contents of any files whose diffs are omitted because they are binary or exceed the preview limits. Use the printed apply command after approval; `--diff` cannot be combined with `--apply` or `--json`.
