...
In this tutorial you will install a plugin after its Agent Plugin Forge pull request has merged. The marketplace was already configured when you installed the Forge, so no repository clone or second marketplace setup is needed.
...
Run **Chat: Open Customizations**, choose **Plugins**, and select **Update Plugins** to refresh configured marketplaces. Choose **Browse Marketplace**, select `PLUGIN_NAME`, and choose **Install**. Return to the installed Plugins list and confirm that it is enabled.
