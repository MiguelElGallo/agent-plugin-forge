...
from .gitflow import create_skill_branch, validate_pr_scope, validate_skill_branch
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="forge", description="Package Agent Skills safely")
    ...
    branch_name = subparsers.add_parser("branch-name", help="Validate a PR skill branch name")
    branch_name.add_argument("--branch", required=True)

    pr_scope = subparsers.add_parser("pr-scope", help="Validate a PR diff against its skill branch")
...
def _request(args: argparse.Namespace) -> ImportRequest:
    ...
    plugin = args.plugin or source.name
